...
class SyncBatchNorm(torch.nn.Module):
  ...
  def forward(self, batch: torch.Tensor) -> torch.Tensor:
    ...
    if self.training:
      ...
      mean = all_reduce_sum_with_backward(local_mean) * scale
      sqr_mean = all_reduce_sum_with_backward(local_sqr_mean) * scale
...
def all_reduce_sum_with_backward(tensor):
  return AllReduceSumLayer.apply(tensor)
