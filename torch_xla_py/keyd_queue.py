...
class Queue(QueueBase):
  ...
  def __init__(self, maxsize=1024):
    ...
    # Trying 2X maxsize, since max size is currently also used in the
    # _get_batch() method as the enqueing upper limit to even if two
    # threads are feeding the queueing will be blocking.
    self._items = queue.Queue(maxsize=2 * maxsize)


# Using the barebones queue.Queue implementation gives us ~290 ex/sec/core
  def put(self, item):
...
# This manual implementation gives us about the same: ~280 ex/sec/core
#  def put(self, item):
#    with self._lock:
