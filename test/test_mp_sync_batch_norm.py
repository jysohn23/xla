...
def sync_bn1d_no_channel_test(index):
  ...
  # TODO(jysohn): delete below
  a_diff = (cpu_result - expected).abs()
  r_diff = a_diff / expected
  print(
      f'a_diff: {a_diff.max()}, {(a_diff > ATOL).count_nonzero()}/{a_diff.numel()}'
  )
  print(
      f'r_diff: {r_diff.max()}, {(r_diff > RTOL).count_nonzero()}/{r_diff.numel()}'
  )
  # TODO(jysohn): delete above
